...
MPI_NPROCS="${MPI_NPROCS:-4}"
RMCDHF_VARIED="${RMCDHF_VARIED:-*}"
RMCDHF_SPECTROSCOPIC="${RMCDHF_SPECTROSCOPIC:-*}"
...
if [[ $CASE =~ ^mpi ]]; then
	...
	RANGULAR="mpirun -n ${MPI_NPROCS} ${GRASP_BINDIR}/rangular_mpi"
	RMCDHF="mpirun -n ${MPI_NPROCS} ${GRASP_BINDIR}/rmcdhf_mpi"
else
