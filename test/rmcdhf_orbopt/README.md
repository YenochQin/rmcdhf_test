...
Production active-space runs can additionally use the immutable-anchor and
acceptance transaction described in
`test/rmcdhf_orbopt/STAGE_GUARD_IMPLEMENTATION.md`.  The short form is:

```bash
GRASP_STAGE_TRANSACTION=1 \
GRASP_STAGE_CONFIG=test/rmcdhf_orbopt/fixtures/ni_target_acceptance.json \
GRASP_STAGE_RCI_STDIN=/path/to/fixed-rci.stdin \
GRASP_ROUND_TARGET_STATES=5,4,3 \
bash test/rmcdhf_orbopt/run_data_case.sh ni_i balanced /absolute/results/case 46 estimate 2
```

The public production entry point is generated by
`graspkit-tools/scripts/grasp_regular_cal/generate_grasp_regular_scripts.py`
and executes `run_orbopt_stage.py` once per increasing active space.  The
`GRASP_STAGE_TRANSACTION` branch above remains an archived-data regression
adapter and imports the same guard implementation from the paired Tools repo.

Unbalanced relativistic partner selection is now rejected by the production
runner.  A deliberate one-sided experiment must set
`GRASP_ALLOW_UNBALANCED=1`; that override is recorded in
`selection_manifest.json` and does not make the run production-acceptable.
