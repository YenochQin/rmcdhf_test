...
!-----------------------------------------------
      ...
      USE WAVE_C
      USE MPI_C
      USE ORBOPT_TRACE_C, ONLY: TRACE_ORTHY_ORDER,                  &
                                TRACE_ORTHY_PROJECTION
!-----------------------------------------------
...
! The fixed orbitals are not changed
      ...
      CALL TRACE_ORTHY_ORDER(0, NAKJ, KTOTAL, KINDX)
      ...
      DO LRAW = KFIXED + 1, KTOTAL
         ...
         DO KRAW = 1, LRAW - 1
            K = KINDX(KRAW)
            OVRLAP = RINT(L,K,0)
            CALL TRACE_ORTHY_PROJECTION(0, KRAW, L, K, OVRLAP)
